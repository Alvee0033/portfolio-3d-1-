...
function createProjectsContent() {
  // Load duck model (simplified for this example)
  const group = new THREE.Group()
    ...
    case "skills":
      scene.children.forEach((child) => {
        if (
          ...
          child.children.some((c) => c instanceof THREE.Mesh && c.geometry instanceof THREE.BoxGeometry)
        ) {
          child.visible = true
          child.userData = { section: "skills" }
        }
      })
      break
    case "education":
      scene.children.forEach((child) => {
        if (
          child instanceof THREE.Group &&
          child.children.some((c) => c instanceof THREE.Mesh && c.geometry instanceof THREE.CylinderGeometry)
        ) {
          child.visible = true
          child.userData = { section: "education" }
        }
      })
      break
    case "projects":
      scene.children.forEach((child) => {
        if (child instanceof THREE.Group && child.children.some((c) => c instanceof THREE.PointLight)) {
          child.visible = true
          child.userData = { section: "projects" }
        }
      })
      break
    case "contact":
      scene.children.forEach((child) => {
        if (child instanceof THREE.Mesh && child.geometry instanceof THREE.SphereGeometry && child.material.wireframe) {
          child.visible = true
          child.userData = { section: "contact" }
        }
      })
      break
  }
