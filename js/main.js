document.addEventListener("DOMContentLoaded", () => {
  ...
  // Function to update scene (defined in three-scene.js)
  function updateScene(sectionId) {
    // This function is implemented in three-scene.js
    // It's declared here to avoid undefined errors
    if (window.updateThreeScene) {
      window.updateThreeScene(sectionId)
    }
  }
  ...
  function setActiveSection(sectionId) {
    ...
    // Update URL hash for direct linking (GitHub Pages friendly)
  ...
  // Handle direct navigation via URL hash (for GitHub Pages)
  function handleHashNavigation() {
    const hash = window.location.hash.substring(1)
    if (hash && ["bio", "skills", "education", "projects", "contact"].includes(hash)) {
      setActiveSection(hash)
    }
  }

  // Check for hash navigation on load
